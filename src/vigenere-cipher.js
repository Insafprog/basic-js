...
class VigenereCipheringMachine {
  ...
  encrypt(message, key) {
    ...
    const result = messageArray.map((element, i) => {
      const number = (this.indexes[element.toUpperCase()] + this.indexes[keyArray[this.index%keyArray.length].toUpperCase()])%26
  ...
  decrypt(message, key) {
    ...
    const keyArray = key.split("")
    const result = messageArray.map((element, i) => {
      const number = (this.indexes[element.toUpperCase()] - this.indexes[keyArray[this.index%keyArray.length].toUpperCase()] + 26)%26
      if (isNaN(number)) return element
      this.index++
      return this.keys[number]
    })
